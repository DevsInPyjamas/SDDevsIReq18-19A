...
public class ApadrinarJoven {
    ...
    public void save() {
        DBManager dbManager = new DBManager();
        if(this.id != 0) {
            String fechaQueryString = (fecha_fin == null) ? "fecha_fin = null" : "fecha_fin = '" + this.fecha_fin + "'";
            dbManager.execute("Update ApadrinarJoven set" +
                    " apadrinador_id = " + this.apadrinador_id +
                    ", joven_id = " + this.joven_id + ", cuota = " + this.cuota + ", " + fechaQueryString + " where id = " + this.id + ";");
        } else {
